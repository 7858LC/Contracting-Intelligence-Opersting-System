...
_RATE_LIMIT_PASSWORD = "RateLimitTest123!"


async def _register(client: AsyncClient) -> dict:
    """Award Simulator is gated Growth+ via require_feature()
    (api/v1/router.py), and some tests below exercise it — register always
    issues plan="trial", so this bumps the tenant's plan and logs in again
    for a token that actually carries it (see conftest's
    upgrade_tenant_plan)."""
    ...
    login = await client.post(
        "/api/v1/auth/login",
        json={"email": email, "password": _RATE_LIMIT_PASSWORD},
        headers=ip_header,
    )
