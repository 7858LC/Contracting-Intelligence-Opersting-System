...
async def _register_professional(client: AsyncClient) -> dict:
    ...
    again for a token that actually carries it — see conftest's
    upgrade_tenant_plan for why a DB update alone isn't enough."""
    ...
    login = await client.post(
        "/api/v1/auth/login",
        json={"email": email, "password": _SMOKE_PASSWORD},
        headers=ip_header,
    )
