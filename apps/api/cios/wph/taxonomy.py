...
SIGNAL_LEXICON: tuple[SignalPattern, ...] = (
    SignalPattern(
        ...
        ("transition", "phase-in", "phase in", "onboarding", "continuity of operations",
         "no disruption", "seamless", "ramp-up", "ramp up", "knowledge transfer"),
        80.0,
    ...
    SignalPattern(
        ...
        ("past performance", "cpars", "relevant experience", "demonstrated experience",
         "recency and relevancy", "corporate experience", "references"),
        75.0,
        "Past performance is an explicit evaluation factor; relevant, recent CPARS matter.",
    ),
    SignalPattern(
        SignalCategory.SECURITY_REQUIREMENT,
        ("security clearance", "top secret", "ts/sci", "secret clearance", "facility clearance",
         "fcl", "cmmc", "nist 800-171", "fedramp", "classified", "cui", "personnel security"),
        85.0,
    ...
    SignalPattern(
        ...
        ("lowest price", "lpta", "price reasonableness", "cost realism", "best value",
         "price competitive", "cost-effective", "affordability"),
        70.0,
        "Price is a meaningful evaluation lever; cost competitiveness affects positioning.",
    ),
    SignalPattern(
        SignalCategory.KEY_PERSONNEL,
        ("key personnel", "program manager", "project manager", "named personnel",
         "resume", "labor category", "certified", "pmp", "key staff"),
        72.0,
        "Named/qualified key personnel are evaluated; specific credentials are expected.",
    ),
    SignalPattern(
        SignalCategory.STAFFING_REQUIREMENT,
        ("full-time equivalent", "fte", "staffing plan", "level of effort", "labor hours",
         "recruit", "retention", "workforce"),
        60.0,
        "Staffing depth and a credible workforce plan are required.",
    ),
    SignalPattern(
        SignalCategory.SCALE_CAPACITY,
        ("nationwide", "enterprise-wide", "surge", "scalability", "large scale",
         "multiple locations", "24/7", "high volume", "capacity"),
        65.0,
        "Scale and surge capacity are required; small-capacity firms are disadvantaged.",
    ),
    SignalPattern(
        SignalCategory.TECHNICAL_COMPLEXITY,
        ("technical approach", "system integration", "architecture", "engineering",
         "complex", "technical solution", "sophisticated", "cloud migration",
    ...
    SignalPattern(
        ...
        ("innovation", "modernization", "emerging technology", "artificial intelligence",
         "machine learning", "automation", "digital transformation", "novel approach"),
        62.0,
        "Government seeks innovation/modernization; differentiated approaches are rewarded.",
    ),
    SignalPattern(
        SignalCategory.DOMAIN_EXPERTISE,
        ("subject matter expert", "domain expertise", "mission knowledge",
         "agency mission", "specialized knowledge", "deep understanding", "sme"),
        70.0,
        "Deep domain/mission expertise is expected; generalists are disadvantaged.",
    ),
    SignalPattern(
        SignalCategory.COMPLIANCE_REQUIREMENT,
        ("iso 9001", "cmmi", "section 508", "accessibility", "regulatory compliance",
         "quality management", "far compliance", "certification required", "compliance with"),
        58.0,
        "Formal compliance/quality certifications are required or strongly preferred.",
    ),
    SignalPattern(
        SignalCategory.SMALL_BUSINESS_PREFERENCE,
        ("small business", "8(a)", "8a", "sdvosb", "hubzone", "wosb", "set-aside",
         "set aside", "socioeconomic", "disadvantaged business"),
        75.0,
        "A set-aside or small-business preference gates eligibility to qualifying firms.",
    ),
    SignalPattern(
        SignalCategory.SCHEDULE_PRESSURE,
        ("aggressive schedule", "tight timeline", "immediate", "expedited", "urgent",
         "short turnaround", "accelerated", "period of performance begins"),
        60.0,
        "Schedule pressure elevates the value of a ready, low-risk offeror.",
    ),
    SignalPattern(
        SignalCategory.GEOGRAPHIC_REQUIREMENT,
        ("on-site", "on site", "place of performance", "local presence", "within",
         "co-located", "government facility", "region"),
        50.0,
        "On-site/geographic presence is required; local footprint helps.",
    ),
    SignalPattern(
        SignalCategory.AGENCY_PRIORITY,
        ("strategic priority", "agency goal", "mission critical", "highest priority",
         "administration priority", "executive order", "strategic objective"),
        66.0,
        "The buy advances an explicit agency/administration priority; mission alignment matters.",
    ),
    SignalPattern(
        SignalCategory.EVALUATION_EMPHASIS,
        ("most important", "significantly more important", "weighted", "evaluation factor",
         "adjectival rating", "technically acceptable", "trade-off", "tradeoff",
         "factors are", "in descending order"),
        72.0,
        "Explicit evaluation weighting signals where the government will discriminate.",
    ),
    SignalPattern(
        SignalCategory.REQUIREMENT_AMBIGUITY,
        ("to be determined", "tbd", "further guidance", "will be provided later",
         "not yet defined", "under development", "clarification"),
        45.0,
...
ATTRIBUTE_LIBRARY: tuple[AttributeDef, ...] = (
    AttributeDef(
        "transition_capability", "Low-Risk Transition Capability", "transition",
        "Proven ability to phase in and assume operations without disruption to the mission.",
        14.0,
        (SignalCategory.TRANSITION_RISK, SignalCategory.INCUMBENT_ADVANTAGE,
         SignalCategory.SCHEDULE_PRESSURE),
        ("transition", "phase-in", "onboarding", "continuity", "knowledge transfer"),
    ),
    AttributeDef(
        "relevant_past_performance", "Relevant & Recent Past Performance", "past_performance",
        "A portfolio of recent, relevant contracts of similar size, scope, and complexity.",
        16.0,
        (SignalCategory.PAST_PERFORMANCE_EMPHASIS, SignalCategory.EVALUATION_EMPHASIS,
         SignalCategory.INCUMBENT_ADVANTAGE),
        ("past performance", "cpars", "contract", "delivered", "prime", "experience"),
    ),
    AttributeDef(
        "security_posture", "Security & Clearance Posture", "security",
    ...
    AttributeDef(
        "technical_solution_depth", "Technical Solution Depth", "technical",
        ...
        ("engineering", "architecture", "integration", "cloud", "cybersecurity",
         "analytics", "software", "devsecops"),
    ),
    AttributeDef(
        "domain_mission_expertise", "Domain & Mission Expertise", "domain",
    ...
    AttributeDef(
        "qualified_key_personnel", "Qualified Key Personnel", "personnel",
    ...
    AttributeDef(
        "staffing_scale", "Staffing Scale & Surge Capacity", "capacity",
    ...
    AttributeDef(
        "cost_competitiveness", "Cost Competitiveness", "price",
    ...
    AttributeDef(
        "set_aside_eligibility", "Set-Aside / Socioeconomic Eligibility", "eligibility",
    ...
    AttributeDef(
        "compliance_readiness", "Regulatory & Compliance Readiness", "compliance",
    ...
    AttributeDef(
        "innovation_modernization", "Innovation & Modernization Capability", "innovation",
    ...
    AttributeDef(
        "geographic_presence", "Geographic / On-Site Presence", "geographic",
